...
import { parseBehaviorFile, parseFoundationFile, parseAppendixFile, parseOverviewFile, parseHomeworkFile, } from "./content-parser";
/**
 * Compiles all markdown files in the docs directory into a structured TrainingLevelsData dataset.
 */
export function compileTrainingLevels(docsDir) {
    ...
    const foundationFiles = fs
        ...
        .sort();
    const foundations = foundationFiles.map((file, idx) => {
    ...
    const levels = levelDirs.map(({ level, dir, title }) => {
        ...
        // Behaviors
        const behaviorFiles = files.filter((f) => !f.includes("overview") && !f.includes("homework"));
        const behaviors = behaviorFiles.map((file, idx) => {
    ...
    const appendixFiles = fs
        ...
        .filter((f) => f.endsWith(".md"))
        .sort();
    const appendices = appendixFiles.map((file, idx) => {
