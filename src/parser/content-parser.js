...
import { CalloutTypeSchema, } from "../types/curriculum";
/**
...
export function parseCallouts(markdown) {
    ...
    while ((match = regex.exec(markdown)) !== null) {
        ...
        const type = parsedType.success ? parsedType.data : "note";
 ...
 */
export function parseCriteriaTable(markdown) {
    const rows = [];
    // Table match: | **Step 1** | ... |
 ...
 */
function extractSectionByHeading(markdown, headingPattern) {
 ...
 */
export function parseBehaviorFile(markdown, meta) {
    ...
    const comebefores = extractSectionByHeading(introText, new RegExp(`(?:^|\\n)(?:###?\\s+Comebefores|\\*\\*Comebefores:?\\*\\*)([\\s\\S]*?)${sectionDelimiter}`, "i")) || null;
    const equipment = extractSectionByHeading(introText, new RegExp(`(?:^|\\n)###?\\s+Equipment([\\s\\S]*?)${sectionDelimiter}`, "i")) || null;
    const thinkAbout = extractSectionByHeading(introText, new RegExp(`(?:^|\\n)###?\\s+Think [Aa]bout[^\\n]*([\\s\\S]*?)${sectionDelimiter}`, "i")) || null;
    const aboutCues = extractSectionByHeading(introText, new RegExp(`(?:^|\\n)###?\\s+About the cues([\\s\\S]*?)${sectionDelimiter}`, "i")) || null;
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        // Extract Try It Cold
        const tryItColdMatch = stepSection.match(/-\s*\[\s*\]\s*\*\*Try It Cold:?\*\*:?\s*(.*?)(?=\n-\s*\[\s*\]|\n---|\n##|\Z)/is);
        const tryItCold = tryItColdMatch ? tryItColdMatch[1].trim() : null;
        // Extract Comeafters
        const comeaftersMatch = stepSection.match(/-\s*\[\s*\]\s*\*\*Comeafters:?\*\*:?\s*(.*?)(?=\n-\s*\[\s*\]|\n---|\n##|\Z)/is);
 ...
 */
function parseSectionFile(markdown, filename, order, defaultSection) {
    const { data, body } = parseFrontmatter(markdown);
 ...
 */
export function parseFoundationFile(markdown, filename, order) {
    return parseSectionFile(markdown, filename, order, "Foundations");
}
/**
 * Parses an appendix markdown file.
 */
export function parseAppendixFile(markdown, filename, order) {
 ...
 */
function parseLevelDocFile(markdown, level, docType) {
    const { data, body } = parseFrontmatter(markdown);
 ...
 */
export function parseOverviewFile(markdown, level) {
    return parseLevelDocFile(markdown, level, "Overview");
}
/**
 * Parses a homework markdown file.
 */
export function parseHomeworkFile(markdown, level) {
