...
async function main() {
    ...
    const totalBehaviors = trainingData.levels.reduce((sum, l) => sum + l.behaviors.length, 0);
    const totalSteps = trainingData.levels.reduce((sum, l) => sum + l.behaviors.reduce((bSum, b) => bSum + b.steps.length, 0), 0);
